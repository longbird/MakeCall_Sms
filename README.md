...
3. **수신 전화 자동 거부**: 수신 전화를 자동으로 감지하여 즉시 거부 처리
4. **세분화된 통화 상태 기록**: 8가지 통화 상태를 구분하여 정확히 기록 (연결됨, 없는 번호, 통화 중 등)
5. **SMS 자동 수신 및 저장**: 수신된 모든 SMS를 로컬 데이터베이스와 원격 서버에 저장 (한글 지원)
6. **히스토리 보기**: 저장된 모든 SMS 기록을 시간순으로 확인
7. **SQLite 데이터베이스**: 로컬 데이터베이스에 영구적으로 저장
8. **REST API 연동**: 원격 서버와 통화 상태 및 SMS 기록 동기화
